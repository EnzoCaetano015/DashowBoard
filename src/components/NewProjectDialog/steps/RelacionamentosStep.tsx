...
export const RelacionamentosStep = ({ repositorios }: { repositorios: RepositorioGitHub[] }) => (
    <div className="space-y-3">
        <p className="text-sm text-muted-foreground">
            Relacione serviços mockados aos repositórios reais selecionados. Nada será persistido nesta etapa.
        </p>
        {servicosDisponiveis.slice(0, 4).map((servico) => (
            <div key={servico.id} className="flex flex-wrap items-center gap-3 rounded-md border border-border bg-surface-2 p-3">
                ...
                <Select defaultValue="sem-repositorio">
                    <SelectTrigger className="min-w-48 bg-surface-3"><SelectValue /></SelectTrigger>
                    <SelectContent>
                        <SelectItem value="sem-repositorio">— sem repositório —</SelectItem>
                        {repositorios.map((repositorio) => <SelectItem key={repositorio.id} value={String(repositorio.id)}>{repositorio.fullName}</SelectItem>)}
                    </SelectContent>
                </Select>
            </div>
        ))}
